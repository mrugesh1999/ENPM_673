...
video_data = cv2.VideoCapture('/home/zeus/Desktop/Perception/Ball_travel_2_updated.mp4')
...
plot_graph('-1 * ((a*(x**2))+(b*x)+c)', range(0, 3400), 'LS', linewidth='1.0')
...
alfa, beta, gama, best_inliner, best_parameter = ransac(myPoints, 2500, 150)

# Plotting graph based on RANSAC
plot_graph('-1 * ((alfa*(x**2))+(beta*x)+gama)', range(0, 3400), 'RANSAC')

# Uncomment the portion below to apply LS method to only the inliners
...
plot_graph('(aye*(x**2))+(bee*x)-d', range(0, 3400), 'TLS')
